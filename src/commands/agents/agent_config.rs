...
pub const GOTCHAS: &[&str] = &[
    ...
    "--llm accepts any string but the Agents backend has its own allowlist. Discover the live list \
     with `elevenlabs agents llms`. This CLI defaults to `gemini-3.1-flash-lite`, the stable \
     replacement reported by the live catalog for the deprecated preview. If an accepted LLM \
     fails to generate at conversation time (0 output \
